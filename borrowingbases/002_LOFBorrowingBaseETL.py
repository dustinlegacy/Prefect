...
@task
def extract_data_lof (dataset_url: str) -> pd.DataFrame:
    #importing the file
...
@task
def transform_data_lof (df):
    df = df.rename(columns = {'Unnamed: 1':'crnumber', 'Unnamed: 2':'loannumber', 'Unnamed: 4':'builder', 'Unnamed: 5':'propertyaddress', \
                              'Unnamed: 6':'code', 'Unnamed: 7': 'units', 'Unnamed: 8': 'city', 'Unnamed: 9': 'state', 'Unnamed: 10': 'county', 'Unnamed: 11':'zip', \
                                'Unnamed: 12' :'sfr', 'Unnamed: 13': 'multiunit', 'Unnamed: 14' : 'notedate', 'Unnamed: 15':'noteexpiry', 'Unnamed: 16' : 'pledgedate',\
                                'Unnamed: 17': 'removedate', 'Unnamed: 18' : 'availableborrowingbase', 'Unnamed: 19': 'noteamount', 'Unnamed: 20': 'purchaseprice', 'Unnamed: 21': 'propertyvalue',\
    ...
    condition = (df['purchaseprice'] == "Line of Credit and Available Borrowing Base as of:") | (df['purchaseprice'] == "Total Borrowing Base") | (df['purchaseprice'] == "Line of Credit and Available Borrowing Base as")
    
    first_occurrence = df[condition].index[0]
    
    df = df.iloc[first_occurrence:]
    
    columns_to_keep = ["purchaseprice", "programlimit"]
...
def load_data_lof (df):
    ...
    metadata = MetaData ()
    b_lofborrowingbase = Table ('b_lofborrowingbase', metadata,
    ...
    df.to_sql('b_lofborrowingbase', con = conn, if_exists = "replace", index=False,
...
@flow
def etl_lof():
    dataset_url = r'C:\data-analysis\_shared\b_excel_files\3b_lofborrowingbase\LOFBorrowingBase(Current).xlsx'
    df = extract_data_lof (dataset_url)
    df_transformed = transform_data_lof (df)
    df_load = load_data_lof (df_transformed)
